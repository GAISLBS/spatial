...
public class ShapefileImporter implements Constants {
    ...
    public List<Node> importFile(String dataset, String layerName, Charset charset) throws IOException {
        ...
        try (Transaction tx = database.beginTx()) {
            layer = (EditableLayerImpl) spatialDatabase.getOrCreateLayer(tx, layerName, WKBGeometryEncoder.class, layerClass);
            tx.commit();
        }
                                    ...
                                    try {
                                        ...
                                        if (filterEnvelope == null || filterEnvelope.intersects(geometry.getEnvelopeInternal())) {
                                            ...
                                            if (geometry.isEmpty()) {
                                                ...
                                                // ?
                                                SpatialDatabaseRecord spatial_record = layer.add(tx, geometry, fieldsName, fields.toArray(values));
